...
Notes:
  - -i is interpreted by step type:
      * monocle3 / rds2h5ad / scenic / length: Seurat .rds input
...
if [[ ${#OUT_PATHS[@]} -gt 0 ]]; then
  ...
  # only set derived monocle3 output if caller didn't already provide a concrete .rds path above
  if [[ ! " ${OV_SHORT[*]} " =~ " out_obj_rds=" ]]; then
    OV_SHORT+=("out_obj_rds=$od/obj_with_pseudotime.rds")
  fi
  OV_SHORT+=("out_deg_csv=$od/deg_graph_test.csv" "out_pr_deg_ids=$od/pr_deg_ids.txt" "out_gene_modules_csv=$od/gene_modules.csv")
...
if [[ ${#IN_PATHS[@]} -gt 0 ]]; then
  ip="${IN_PATHS[-1]}"
  OV_SHORT+=("input_rds=$ip")
  OV_SHORT+=("rds2h5ad_input_rds=$ip")

  # When monocle3 is NOT in the requested steps, downstream RDS-based steps should
  # respect -i directly instead of preferring out_obj_rds from the output dir.
