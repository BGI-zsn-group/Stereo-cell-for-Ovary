...
def main(args):
    ...
    if len(args.normalize) % 2 != 0:
        ...
        quit()
    if args.function != 'split' and args.function != 'all' and args.function != 'split' and args.function != 'merge' and args.function != 'nCount':
    ...
    def cm_RGB(x, i_min, i_max):
        return plt.cm.get_cmap(args.cmap)((np.clip(x, i_min, i_max) - i_min) / (i_max - i_min))

    if args.function == 'nCount':
        gem_data = gem_data.groupby(['x', 'y']).agg(value=('value', 'sum')).reset_index()
        ...
        if len(args.normalize)!=0:
            for i in range(0,int(len(args.normalize)/2)):
                ...
                plot_data = gem_data.copy()
                plot_data.loc[plot_data['value']<= args.normalize[2*i],['value']] = args.normalize[2*i]
                plot_data.loc[plot_data['value'] >= args.normalize[2*i+1], ['value']] = args.normalize[2*i+1]
                plot_data[['R', 'G', 'B', 'A']] = cm_RGB(plot_data['value'].tolist(), plot_data['value'].min(), plot_data['value'].max())
                coords[plot_data['y'], plot_data['x'],0] = plot_data['R']
                coords[plot_data['y'], plot_data['x'], 1] = plot_data['G']
                coords[plot_data['y'], plot_data['x'], 2] = plot_data['B']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cmap", help="nCount cmap", type=str, default='gray',nargs='?')
